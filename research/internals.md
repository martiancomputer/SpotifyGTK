...
Both applications have artwork caches suited to their UI. `spotify-native`'s
`ui/cover_loader.c` fetches from the image CDN, decodes at the requested size
off the GTK thread, keeps decoded textures in a bounded 24 MB LRU, and stores
compressed images in a bounded 1 GB disk cache. Visible playback artwork has
priority over speculative grid work, cancelled/recycled rows stop consuming
decode work, and Settings can disable caching, clear it, or trade decoded
texture retention for higher bounded concurrency. `spotify-connect` retains
its independent VA-API → libjpeg-turbo → stb_image design because the two
applications are built separately.
...
| Audio output: WASAPI (Windows) | ✅ Builds, signs in, streams, and plays on real Windows 11 hardware. Shared-mode `IAudioClient` + `IAudioRenderClient` provides blocking backpressure, `ISimpleAudioVolume` is used when the endpoint offers it, and COM is initialised and torn down on the audio worker. `AUTOCONVERT_PCM` lets a 44.1 kHz stream open on a 48 kHz mixer, with a retry for drivers that reject the flag. The MSYS2 UCRT64 build also compiles and links in CI. See `apps/spotify-native-windows/README.md`. |
...
| Artwork scheduling | Normal mode caps fetch/decode work at eight concurrent jobs and reserves capacity for the playback bar and Now Playing panel. Aggressive mode permits sixteen and drops the decoded-cache reference once the widget releases its own. Scrolling defers row work, then reloads visible covers and prefetches a small forward window when it settles. |
| Persistent media | Compressed artwork is capped at 1 GB on disk; artist-name, portrait, and release mappings are stored as small metadata indexes. Caching can be disabled or cleared live from Settings. |
...
| `SPOTIFY_COVER_STATS=1` | GUI | Logs periodic cover-cache and request-queue statistics. |
...
- [x] Themes: Dark (default), White, Milk and Dark+ — one rule body over four `@define-color` palettes, applied live via a reloaded `GtkCssProvider` + `AdwStyleManager` color-scheme
- [x] Spotify Connect device registration, transfer and remote transport commands, active-device arbitration, and synchronized playback state
- [x] Ordinary shuffle, repeat-one/repeat-all, Connect-backed Smart Shuffle, and restoration of the sequential queue when Smart Shuffle is disabled
- [x] Search radio queues up to a 200-track deduplicated target and up to 150 ranked search tracks
- [x] Native account profile in Settings: display name, canonical id, product tier and avatar
- [x] Media controls: full artwork, Now Playing only, or none; optional persistent caching; aggressive bounded loading; and live cache clearing
- [x] Library separation for albums, EPs, singles and followed artists, with local filtering and persistent artist metadata indexes
- [x] App-wide network activity indicator and opt-in compiled performance diagnostics
...
- [x] Selectable GTK/GSK compositor — Automatic, Vulkan, OpenGL or Cairo, persisted with restart semantics and environment override
