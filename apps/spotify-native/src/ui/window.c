...
/* From the pinned list's own menu, so there is nothing to toggle: the row
 * exists because the thing is pinned. */
static void
on_sidebar_unpin_requested (SpotifyGtkSidebar *sidebar, const gchar *uri,
                            gpointer user_data)
{
  SpotifyGtkNativeWindow *self = user_data;
  (void) sidebar;
  if (!uri || !*uri)
    return;
  spotifygtk_settings_remove_pin (spotifygtk_settings_get_default (), uri);
  refresh_pinned_sidebar (self);
}
...
spotifygtk_native_window_constructed (GObject *object)
{
  ...
  g_signal_connect (self->sidebar, "unpin-requested",
                    G_CALLBACK (on_sidebar_unpin_requested), self);
