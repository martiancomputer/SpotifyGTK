/*
 * native_auth.c — OAuth PKCE flow for the AP-level (keymaster)
 * credential, ported from apps/spotify-connect/src/auth.c's real,
 * already-working PKCE machinery. See native_auth.h for why this is
 * a separate module and where every constant came from.
 *
 * Differences from spotify-connect's auth.c, all deliberate:
 *   - Client ID, scopes, and redirect are hardcoded constants here
 *     (NATIVE_AUTH_CLIENT_ID / NATIVE_AUTH_SCOPES / NATIVE_AUTH_REDIRECT_URI),
 *     not read from an env var / dashboard registration -- there's
 *     nothing to register, this client_id is Spotify's own.
 *   - Token storage: spotify-native has no libsecret dependency
 *     (checked -- it's not in this app's meson.build; only glib/gio/
 *     soup/ogg/vorbis/openssl/audio-backends). Rather than add a new
 *     dependency for what's currently a CLI dev harness, this stores
 *     to a plaintext, chmod 600 file, same fallback path
 *     spotify-connect already uses when libsecret is unavailable
 *     there. Revisit if/when spotify-native grows a persistent
 *     background-process story where that tradeoff matters more.
 */
...
G_DEFINE_FINAL_TYPE (NativeAuth, native_auth, G_TYPE_OBJECT)

/* ── PKCE helpers -- identical logic to spotify-connect's auth.c ────────────── */
...
/* ── Token storage -- plaintext chmod 600 file, see file header ─────────────── */
...
on_token_response (GObject *source, GAsyncResult *result, gpointer user_data)
{
  ...
  if (!json_parser_load_from_data (parser, body, (gssize) len, &err)) {
    g_warning ("native_auth: JSON parse error: %s", err->message);
    g_signal_emit (self, signals[SIG_COMPLETED], 0, FALSE);
    g_bytes_unref (bytes);
    return;
  }
...
static void
exchange_code (NativeAuth *self, const gchar *code)
{
  ...
  g_autofree gchar *body =
    g_strdup_printf ("grant_type=authorization_code&code=%s&redirect_uri=%s"
                     ...
                     code, NATIVE_AUTH_REDIRECT_URI, NATIVE_AUTH_CLIENT_ID, self->code_verifier);
 ...
 * Path is "/login" here, not "/callback" -- must match
 * NATIVE_AUTH_REDIRECT_URI exactly, which itself must match whatever
 * is pre-registered on Spotify's side against the keymaster client_id
 * (verified against librespot-oauth's own worked example, see
 * native_auth.h). */
                     ...
                     GHashTable *query, gpointer user_data)
{
  ...
  const gchar *html_err =
    "<html><body style='font-family:sans-serif;text-align:center;padding:4em'>"
    "<h2>Authentication failed</h2><p>Check the terminal for details.</p></body></html>";

  if (error || !code) {
    g_warning ("native_auth: OAuth callback error: %s", error ? error : "no code");
  ...
  if (g_strcmp0 (state, self->state_nonce) != 0) {
    g_warning ("native_auth: OAuth state mismatch, possible CSRF attempt");
    soup_server_message_set_status (msg, 400, NULL);
    soup_server_message_set_response (msg, "text/html", SOUP_MEMORY_STATIC, html_err, strlen (html_err));
    g_signal_emit (self, signals[SIG_COMPLETED], 0, FALSE);
    goto cleanup;
...
gboolean
native_auth_has_valid_token (NativeAuth *self)
{
...
const gchar *
native_auth_get_token (NativeAuth *self)
{
  g_return_val_if_fail (NATIVEAUTH_IS_AUTH (self), NULL);
...
native_auth_begin (NativeAuth *self)
{
  ...
  g_autofree gchar *encoded_scopes = g_uri_escape_string (NATIVE_AUTH_SCOPES, NULL, FALSE);
  g_autofree gchar *encoded_redirect = g_uri_escape_string (NATIVE_AUTH_REDIRECT_URI, NULL, FALSE);
  ...
  g_autofree gchar *url =
    g_strdup_printf ("%s?response_type=code&client_id=%s&scope=%s&redirect_uri=%s"
                     ...
                     SPOTIFY_AUTH_URL, NATIVE_AUTH_CLIENT_ID, encoded_scopes, encoded_redirect,
...
void
native_auth_refresh (NativeAuth *self)
{
  g_return_if_fail (NATIVEAUTH_IS_AUTH (self));
  ...
  g_autofree gchar *body =
    g_strdup_printf ("grant_type=refresh_token&refresh_token=%s&client_id=%s",
                     self->refresh_token, NATIVE_AUTH_CLIENT_ID);
...
static void
native_auth_finalize (GObject *object)
{
  NativeAuth *self = NATIVEAUTH_AUTH (object);
  g_free (self->code_verifier);
  g_free (self->state_nonce);
  g_free (self->access_token);
  g_free (self->refresh_token);
  G_OBJECT_CLASS (native_auth_parent_class)->finalize (object);
}

static void
native_auth_class_init (NativeAuthClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  object_class->dispose  = native_auth_dispose;
  object_class->finalize = native_auth_finalize;
...
NativeAuth *
native_auth_new (void)
{
  return g_object_new (NATIVEAUTH_TYPE_AUTH, NULL);
}
