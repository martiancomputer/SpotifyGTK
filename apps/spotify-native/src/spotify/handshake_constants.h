/*
 * handshake_constants.h -- verified constants for the AP handshake.
 *
 * Both arrays below were extracted programmatically (regex over the
 * raw source, not retyped by hand) from librespot's actual source:
 *   - AP_SERVER_KEY   from core/src/connection/handshake.rs (SERVER_KEY)
 *   - DH_PRIME_BYTES  from core/src/diffie_hellman.rs (DH_PRIME)
 *
 * DH_PRIME_BYTES is the standard RFC 2409 "Second Oakley Group"
 * 768-bit MODP prime (generator = 2) -- a well-known public group, not
 * anything Spotify-specific. AP_SERVER_KEY is Spotify's actual AP
 * service RSA public key (modulus; exponent is the standard 65537),
 * used only to verify the server's handshake signature.
 *
 * See research/auth/ for how these are used.
 */

#pragma once

#include <glib.h>

G_BEGIN_DECLS

#define DH_GENERATOR 2
#define RSA_EXPONENT 65537

static const guint8 AP_SERVER_KEY[256] = {
  ,
};

static const guint8 DH_PRIME_BYTES[96] = {
  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xc9, 0x0f, 0xda, 0xa2,
  0x21, 0x68, 0xc2, 0x34, 0xc4, 0xc6, 0x62, 0x8b, 0x80, 0xdc, 0x1c, 0xd1,
  0x29, 0x02, 0x4e, 0x08, 0x8a, 0x67, 0xcc, 0x74, 0x02, 0x0b, 0xbe, 0xa6,
  0x3b, 0x13, 0x9b, 0x22, 0x51, 0x4a, 0x08, 0x79, 0x8e, 0x34, 0x04, 0xdd,
  0xef, 0x95, 0x19, 0xb3, 0xcd, 0x3a, 0x43, 0x1b, 0x30, 0x2b, 0x0a, 0x6d,
  0xf2, 0x5f, 0x14, 0x37, 0x4f, 0xe1, 0x35, 0x6d, 0x6d, 0x51, 0xc2, 0x45,
  0xe4, 0x85, 0xb5, 0x76, 0x62, 0x5e, 0x7e, 0xc6, 0xf4, 0x4c, 0x42, 0xe9,
  0xa6, 0x3a, 0x36, 0x20, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
};

G_END_DECLS
