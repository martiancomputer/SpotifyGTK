...
play, pause, resume, switch and respond to the volume slider. Previous/Next,
a play queue, and a right-click track menu (Add to Queue, Go to Album, Go to
Artist) now work too — all driven from the UI's own play-context, since the
engine still plays one track at a time. Seek, shuffle, repeat and like remain
disabled: those need engine or library-write support that does not exist yet.
Home and Library are still laid out and empty. The standalone harness remains
the protocol/playback validation tool. See Project status below for the exact
...
- [ ] Seek support in the engine, so the progress slider can be enabled (the last disabled transport control that needs the engine, not just the UI)
