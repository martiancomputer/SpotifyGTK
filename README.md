...
stack. One exception, stated up front rather than buried: **artist banners come
from Spotify's GraphQL API** (pathfinder), because they are not in the native
protocol at all — see the Artist visuals row in Project status. Tracks play, pause, resume, **seek**, switch, and
...
| Artist visuals — **GraphQL** (`spotify/spclient.c`) | ✅ **This client calls Spotify's GraphQL API,** and that is worth stating plainly rather than filing under "not the Web API". Artist banners are not in the native protocol: the `Artist` message carries `portrait` and `portrait_group` — the square avatar — and nothing resembling a header, across all twenty of its fields. The banner lives on `artistUnion.visuals.headerImage`, served by **pathfinder**, Spotify's GraphQL endpoint at `api-partner.spotify.com/pathfinder/v1/query`. It is a persisted query: the document is never sent, the server already holds it and is addressed by hash, so it cannot be modified without a hash matching whatever it was changed to. Operation name and hash were **probed out of the shipped client's JS bundle** (`new c.l("queryArtistOverview", "query", "1ac33dda…737c72", null)`), and the response shape confirmed from the code that consumes it. Authenticated with this session's own login5 bearer and client-token — the same credentials the desktop client uses, not the shared keymaster ID that caused the 429s above. This is a second catalog transport beside spclient, and the only part of the client that is not the native protocol. |
