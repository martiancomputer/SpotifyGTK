...
stack. Artist pages carry a promo photo rather than the banner the official
desktop client draws — that image is behind an endpoint this client's tokens are
not entitled to, and no native route carries it; see the Artist visuals row in
Project status. Tracks play, pause, resume, **seek**, switch, and
...
| Artist visuals — native (`spotify/session.c`) | ⚠️ **The banner the official desktop client draws is not obtainable, and this is the honest version of a row that twice claimed otherwise.** It lives on `artistUnion.visuals.headerImage`, behind pathfinder (`api-partner.spotify.com`), which answers `403 "Client/request not allowed"` to this client — bearer alone, bearer + client-token, and with `app-platform`/`spotify-app-version` added all fail. Our client id is keymaster, which is not entitled to that endpoint. Nothing else carries it: the full `ExtensionKind` enum has no visuals kind, the shipped binary has no `hm://` artist route and no spclient image path, and `open.spotify.com` is a client-rendered shell with nothing to scrape. What the client shows instead comes from `hm://artistview/v1/artist/<id>` over the AP connection: the backdrop of the pinned "Artist pick" card, a landscape promo photo, falling back to the round avatar when an artist has no pinned pick. Portrait images are fitted rather than covered, so an avatar is never magnified into a crop of a cheek. Earlier versions of this row said the client called Spotify's GraphQL API for this; it did make the call, and the call had never once succeeded. See `apps/spotify-native/research/artist-images.md`. |
