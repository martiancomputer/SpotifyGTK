...
ready (with Log out in Settings), and a local **Liked Songs filter** round out
the shell. Still
...
- [ ] Flatpak packaging
- [~] Windows port — WASAPI backend and the per-OS client-token block written and cross-compile verified; `apps/spotify-native-windows/` holds the cross file, build script and packaging. **Nothing has run on Windows yet**, and a cross `meson setup` cannot reach the platform branches without the mingw dependency stack (MSYS2 build required)
